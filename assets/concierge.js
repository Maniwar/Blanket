...
(function () {
  ...
  function supaKey() {
    /* The studio lets the operator save a working publishable key in this
       ...
       admin's own magic-link session sits in shared storage while the widget,
       stuck on a dead baked key, can neither read nor refresh it. Publishable
       keys only: a secret key must never run in the widget. */
    try {
      var ak = window.localStorage.getItem('feier_admin_key') || '';
      ak = ak.replace(/^\s+|\s+$/g, '');
  ...
  function injectStyle() {
    var css = [
      ...
      /* ---------- scrim ---------- */
      '.cx-scrim{position:fixed;inset:0;z-index:100081;background:rgba(23,31,26,.35);',
  ...
  function buildUI() {
    ...
    /* Help row — the explicit doors. Relying on the model to notice someone is
       ...
       deliberately always present and never buried in the overflow menu — an
       escape hatch you have to hunt for is not an escape hatch. */
    if (remoteSupport) {
      ...
      [
        ...
        ['Share feedback', 'I\u2019d like to share some product feedback.'],
        ['Talk to a person', 'I\u2019d like a person to take this, please.']
      ].forEach(function (pair, idx) {
        var b = el('button', 'cx-help-btn' + (idx === 2 ? ' cx-help-human' : ''), pair[0]);
        b.type = 'button';
        b.addEventListener('click', function () {
          if (!yieldToVisitor()) { return; } /* same rule the reply pills use */
          entryMode = 'help';               /* so attribution shows which door they took */
          sendMessage(pair[1]);
        });
  ...
  /* A visitor's own words outrank a line the bot started on its own.
     ...
     reply pills, the inline page starters, the NPS scale and the help buttons
     each checked `streaming` and returned, with no message, no error and no
     mark on the page. On a site with a lively beat ladder (996's first nudge
     is five seconds after the panel opens) that is most of the taps — the
     listing went three days with proactive lines flowing and not one visitor
     question recorded.
