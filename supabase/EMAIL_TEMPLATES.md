...
<table role="presentation" width="440" cellpadding="0" cellspacing="0" style="width:440px;max-width:92%;background:#232a25;border:1px solid #3a4139;border-radius:14px;overflow:hidden;">
  ...
  <tr><td style="padding:12px 34px 24px;border-top:1px solid #3a4139;font-family:Helvetica,Arial,sans-serif;color:#7f7a6e;font-size:11px;line-height:1.6;">If you didn't request this, you can ignore it. A demo — nothing ships and no payment is taken. Questions: concierge@feier-abend.co</td></tr>
</table></td></tr></table>
```
...
<table role="presentation" width="440" cellpadding="0" cellspacing="0" style="width:440px;max-width:92%;background:#232a25;border:1px solid #3a4139;border-radius:14px;overflow:hidden;">
  ...
  <tr><td style="padding:12px 34px 24px;border-top:1px solid #3a4139;font-family:Helvetica,Arial,sans-serif;color:#7f7a6e;font-size:11px;line-height:1.6;">If this wasn't you, ignore it. A demo — nothing ships and no payment is taken. Questions: concierge@feier-abend.co</td></tr>
</table></td></tr></table>
```
...
setting `RESEND_API_KEY` as a function secret; sending is best-effort and never
blocks the response. The sender (`EMAIL_FROM`) defaults to
`Feierabend <concierge@feier-abend.co>` — an address on the verified Resend domain,
so real recipients receive it. Override it with an `EMAIL_FROM` secret if you send
from a different domain. (Replies to `concierge@feier-abend.co` forward to the
mill's inbox via Cloudflare Email Routing.)
