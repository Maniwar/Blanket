...
| --- | --- | --- |
| Unit tests (`beats_test.ts`) | 28 deterministic tests, red/green (incl. the coach feedback-loop digest + the NPS trigger/score/response-rate math and the analyst-corpus builder) | **Deploy Concierge** run → *Beat engine unit tests* step |
